...
estimator = Pipeline(steps=[('onehot', ce.OneHotEncoder(use_cat_names=True)),
                      ('classifier', RandomForestClassifier(n_estimators=12, max_depth=10, random_state=123)
                      )])
...
mc2.quantitative_analysis.performance_metrics = [mctlib.PerformanceMetric() for i in range(0, 5)]
...
mc2.quantitative_analysis.performance_metrics[4].tests = [mc_tpr_threshold_test_gender, mc_tpr_threshold_test_married]
...
mc2.fairness_analysis.fairness_reports[0].tests = [mc_sgd_test_married, mc_sgd_test_gender]
