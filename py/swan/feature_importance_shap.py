...
def pa_in_top_feature_importance_shap(
    ...
    shap.summary_plot(
        shap_values=shap_values[1], features=x_test, max_display=20, plot_type="dot"
    )
    ...
    agg_shap_df = pd.DataFrame(
        pd.DataFrame(shap_values[1], columns=x_test.columns).abs().mean()
    ...
    for i in result:
        shap.dependence_plot(
            i, shap_values=shap_values[1], features=x_test, interaction_index=None
        )
