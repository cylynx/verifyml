...
class SubgroupDifference(FEATTest):
    ...
    def get_metric_dict(self, attr: str, df: DataFrame) -> dict[str, float]:
        ...
        for value in df[attr].unique():
            ...
            metric_dict[f'{attr}_fnr_max_diff'] =  max(self.fnr.values()) - min(self.fnr.values())
            metric_dict[f'{attr}_fnr_max_ratio'] =  max(self.fnr.values()) / min(self.fnr.values())
            metric_dict[f'{attr}_fpr_max_diff'] =  max(self.fpr.values()) - min(self.fpr.values())
            metric_dict[f'{attr}_fpr_max_ratio'] =  max(self.fpr.values()) / min(self.fpr.values())
            metric_dict[f'{attr}_sr_max_diff'] =  max(self.sr.values()) - min(self.sr.values())
            metric_dict[f'{attr}_sr_max_ratio'] =  max(self.sr.values()) / min(self.sr.values())
