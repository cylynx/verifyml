...
def _generate_graph_from_feature_statistics(
    feature_stats: statistics_pb2.FeatureNameStatistics, color: Optional[Text] = None
) -> Union[_Graph, None]:
...
def _generate_graph_from_slicing_metrics(
    ...
    for slicing_metric in slicing_metrics:
        ...
        if "doubleValue" in metric_value:
            ...
            logging.warning(
                "%s must be a doubleValue or boundedValue; skipping %s.",
                metric,
                slices_key,
            )
            return None
...
SliceKeyType = Union[
    Tuple[()], Tuple[SingletonSliceKeyType, ...]
...
def stringify_slice_key(slice_key: SliceKeyType) -> Tuple[Text, Text]:
    ...
    # ascii codec.
    return (
        separator.join(["{}".format(key) for key in keys]),
