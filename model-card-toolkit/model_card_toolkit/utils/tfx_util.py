...
def _get_one_hop_artifacts(
    ...
    return [
        artifact
        for artifact in store.get_artifacts_by_id(artifacts_ids)
...
def _get_one_hop_executions(
    ...
    return [
        execution
        for execution in store.get_executions_by_id(executions_ids)
...
def get_stats_artifacts_for_model(
    ...
    dataset_ids.update(
        dataset.id
        for dataset in _get_one_hop_artifacts(
            store,
            transformed_example_ids,
            _Direction.ANCESTOR,
...
def read_stats_proto(
    stats_artifact_uri: Text, split: Text
    ...
    feature_stats_path = os.path.join(stats_artifact_uri, split, "FeatureStats.pb")
    stats_tfrecord_path = os.path.join(stats_artifact_uri, split, "stats_tfrecord")
    ...
    if os.path.exists(feature_stats_path):
        ...
        serialized_stats = next(tf.compat.v1.io.tf_record_iterator(stats_tfrecord_path))
        stats.ParseFromString(serialized_stats)
        return stats
    else:
        logging.warning(
            "No artifact found at %s or %s", stats_tfrecord_path, feature_stats_path
        )
        return None


def read_metrics_eval_result(
    metrics_artifact_uri: Text, output_file_format: Optional[Text] = None
