...
class GraphicsTest(parameterized.TestCase):
    ...
    """,
        ...
        self.assertSameElements(
            [g.name for g in train_data.graphics.collection], expected_plot_names_train
        )

        eval_data = model_card.model_parameters.data[1]
        self.assertSameElements(
            [g.name for g in eval_data.graphics.collection], expected_plot_names_eval
        )
    ...
    def test_generate_graph_from_slicing_metrics(self):
        slicing_metrics = [
            (
                (("weekday", 0),),
                {
                    "": {
                        "": {
                            "average_loss": {"doubleValue": 0.07875693589448929},
                            "prediction/mean": {
                                "boundedValue": {
            ...
            (
                (("weekday", 1),),
                {
                    "": {
                        "": {
                            "average_loss": {"doubleValue": 4.4887189865112305},
                            "prediction/mean": {
                                "boundedValue": {
            ...
            (
                (("weekday", 2),),
                {
                    "": {
                        "": {
                            "average_loss": {"doubleValue": 2.092138290405273},
                            "prediction/mean": {"doubleValue": 0.3767518997192383},
                            "__ERROR__": {
                                # CI not computed because only 16 samples
                                # were non-empty. Expected 20.
                                "bytesValue": "Q0kgbm90IGNvbXB1dGVkIGJlY2F1c2Ugb25seSAxNiBzYW1wbGVzIHdlcmUgbm9uLWVtcHR5LiBFeHBlY3RlZCAyMC4="
                            },
                        }
                    }
                },
            ...
            (
                (),
                {
                    "": {
                        "": {
                            "average_loss": {"doubleValue": 1.092138290405273},
                            "prediction/mean": {
                                "boundedValue": {
        ...
        self.assertGraphEqual(
            graphics._generate_graph_from_slicing_metrics(
    ...
    def test_annotate_eval_results_plots(self):
        ...
        expected_metrics_names = {
            "average_loss | weekday",
            "prediction/mean | weekday",
            "average_loss | gender, age",
            "prediction/mean | gender, age",
        }
        self.assertSameElements(
            expected_metrics_names,
    ...
    @parameterized.parameters(
        [
            ...
            [
                ...
                ("gender, zip, height, comment", "male, 12345, 5.7, 你好"),
            ],
        ]
    )
