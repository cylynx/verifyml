...
def get_answer(questions: str, label: str) -> Union[str, None]:
    ...
    if (match['type'] in ['INPUT_TEXT', 'TEXTAREA', 'INPUT_EMAIL']):
        ...
        return [result['text'] for result in filter(lambda d: d['id'] in value_list, match['options'])]
    else:
...
def tally_form_to_mc(form_path: str):
    ...
    mc.model_details.owners = [
        mctlib.Owner(name=get_answer(questions, 'Product Owner(s)'), role='Product Owner(s)'),
        mctlib.Owner(name=get_answer(questions, 'Model Developer(s)'), role='Model Developer(s)'),
        mctlib.Owner(name=get_answer(questions, 'Reviewer(s)'), role='Reviewer(s)'),
    ]
    mc.model_details.regulatory_requirements = ' '.join(get_answer(questions, 'Please select any regulatory guidelines which the model should be in compliance with'))
    ...
    mc.model_parameters.data = [
        mctlib.Dataset(
            ...
            sensitive=mctlib.SensitiveData(
                sensitive_data=str_to_list(get_answer(questions, 'Protected attributes in dataset')),
                sensitive_data_used=str_to_list(get_answer(questions, 'Protected attributes in production')),
                justification=get_answer(questions, 'Justification of use of protected attributes')
            )
        )
    ]
    ...
    if get_answer(questions, 'Name of dataset 2'):
        mc.model_parameters.data.append(
            mctlib.Dataset(
                ...
                sensitive=mctlib.SensitiveData(
                    sensitive_data=str_to_list(get_answer(questions, 'Protected attributes in dataset 2')),
                    sensitive_data_used=str_to_list(get_answer(questions, 'Protected attributes in production (dataset 2)')),
                    justification=get_answer(questions, 'Justification of use of protected attributes (dataset 2)')
                )
            )
        )
    ...
    if get_answer(questions, 'Name of dataset 3'):
        mc.model_parameters.data.append(
            mctlib.Dataset(
                ...
                sensitive=mctlib.SensitiveData(
                    sensitive_data=str_to_list(get_answer(questions, 'Protected attributes in dataset 3')),
                    sensitive_data_used=str_to_list(get_answer(questions, 'Protected attributes in production (dataset 3)')),
                    justification=get_answer(questions, 'Justification of use of protected attributes (dataset 3)')
                )
            )
        )
    ...
    mc.quantitative_analysis.performance_metrics = [
        mctlib.PerformanceMetric(
            type=get_answer(questions, "What is the key metric used in evaluating the model's performance?"),
        )
    ]
    ...
    mc.considerations.fairness_assessment = [
        mctlib.FairnessAssessment(
            group_at_risk=get_answer(questions, 'Who are the individuals and groups that are considered to be at-risk of being systematically disadvantaged by the system?'),
