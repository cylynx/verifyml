...
def get_data() -> Dict[Text, Any]:
    ...
    validation_ds_performant = (
        validation_ds_resized.cache().batch(BATCH_SIZE).prefetch(buffer_size=10)
    )
...
def create_model(training_epochs: int = DEFAULT_TRAINING_EPOCHS) -> tf.keras.Model:
    ...
    train_ds = train_ds.map(resize).cache().batch(BATCH_SIZE).prefetch(buffer_size=10)
    validation_ds = (
        validation_ds.map(resize).cache().batch(BATCH_SIZE).prefetch(buffer_size=10)
    )
